...
all_imgs = np.zeros((1,3,28,28),dtype=np.uint8)
spds = np.zeros(1)
accel = np.zeros(1)
steer = np.zeros(1)
gas = np.zeros(1)
...
for c,l in zip(camfiles,logfiles):
    ...
    smcam = np.zeros((nframes,3,28,28),dtype=np.uint8)
    for i,c in tqdm(enumerate(cams)):
        smcam[i] = scipy.misc.imresize(c[:,:,80:-80],(28,28),'cubic','RGB').transpose((2,0,1))
    ...
    all_imgs.resize((abs_end, 3, 28, 28))
    spds.resize((abs_end))
